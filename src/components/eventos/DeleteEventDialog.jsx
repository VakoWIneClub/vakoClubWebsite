...
const DeleteEventDialog = ({ event, onDeleted, trigger }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({ title: 'Evento eliminado', description: `El evento "${event.title}" ha sido eliminado.` });
      onDeleted(event.id);
    } catch (error) {
      toast({
        variant: 'destructive',
  ...
  return (
    <AlertDialog>
      ...
      {/* AlertDialogContent's base classes bake in .wine-glass-effect (a plain, un-layered CSS
          ...
          it off, same workaround used throughout the copa redesign (Home, Guía, Noticias). */}
      <AlertDialogContent
      ...
      >
        <AlertDialogHeader>
          ...
          <AlertDialogDescription className="text-copa-ink/70" style={{ fontFamily: "'EB Garamond', serif", fontSize: 16 }}>
            Esta acción no se puede deshacer. Esto eliminará permanentemente el evento <strong className="text-copa-burgundy">"{event.title}"</strong> y su imagen asociada.
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>

 ...
 * only a hero, ContactInfo links and FaqSection. These smoke checks cover what
 * actually renders; see the QA notes shared alongside this suite.
 */
test.describe('Contacto page', () => {
  ...
  test('renders the FAQ section', async ({ page }) => {
    ...
    await expect(page.getByText('¿Tienen envío a toda España?')).toBeVisible();
  });
});
